...
model = torch.load('../output/model/kCbert_3multi_0.96_Mar07.pt', map_location=device)    # 업샘플링 학습 모델
# model = torch.load('../output/model/SYS_kCbert_3multi_0.89.pt', map_location=device)   # No 업샘플링 학습 모델
# model.to(device)
...
### 평가 데이터셋 불러오기
df = pd.read_csv('../input/datasets0303/concat_evaluation_datasets_0303.csv', sep="|")
X = df['text']
Y = df['카테고리']
Z = []
...
with open(f'./pickled_ones/kCbert_Z_test.pickle', 'wb') as f:
    pickle.dump(Z, f)
...
print(df_a)
df_a.to_csv(f'./output/정답비교_kCbert.csv', index=False)
...
for a1 in target_names:
    for a2 in target_names:
        if a1 != a2:
            ...
            print(error_df)
            error_df.to_csv(f'./output/kCbert_{a1}_{a2}_error_data.csv', index=False)
